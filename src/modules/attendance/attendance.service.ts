...
import { OpenAttendanceSessionDto, FinalizeAttendanceDto } from './dto/session.dto';
...
function assertGroupLessonWindow(startMin: number, endMin: number) {
  ...
  if (startMin < 0 || startMin > 1439) {
    throw new BadRequestException('lessonStartMin 0..1439 oralig‘ida bo‘lishi kerak');
  }
  if (endMin < 1 || endMin > 1440) {
    throw new BadRequestException('lessonEndMin 1..1440 oralig‘ida bo‘lishi kerak');
  }
  if (startMin >= endMin) {
    throw new BadRequestException('lessonStartMin < lessonEndMin bo‘lishi kerak');
  }
}
...
export class AttendanceService {
  ...
  private assertTeacher(actorId: string, teacherUserId: string) {
    if (!actorId?.trim()) throw new BadRequestException('x-actor-id header yo‘q');
    if (actorId !== teacherUserId) {
      throw new ForbiddenException('Davomatni faqat shu guruh o‘qituvchisi qila oladi');
    }
  }

  // ✅ closeAt o‘tib ketgan bo‘lsa, session DB’da LOCK bo‘ladi
  private async autoLockIfNeeded(tx: Prisma.TransactionClient, sessionId: string) {
    const now = new Date();
  ...
  private assertWindowOpen(session: {
    ...
    const now = new Date();
    if (now < session.openAt) throw new ForbiddenException('Davomat hali ochilmagan');
    if (now >= session.closeAt) throw new ForbiddenException('Davomat yopilgan (dars tugagan)');
  }
  ...
  async openSession(actorId: string, dto: OpenAttendanceSessionDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!group) throw new NotFoundException('Guruh topilmadi yoki aktiv emas');

      this.assertTeacher(actorId, group.teacher.userId);

      assertGroupLessonWindow(group.lessonStartMin, group.lessonEndMin);

      const startMin = group.lessonStartMin;
      const endMin = group.lessonEndMin;
      ...
      if (now >= closeAt) throw new ForbiddenException('Davomat yopilgan (dars tugagan)');
  ...
  async mark(actorId: string, dto: MarkAttendanceDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!active) throw new BadRequestException('Student bu guruhda ACTIVE emas');

      const status = dto.status as unknown as AttendanceStatusStr;

      return tx.attendance.upsert({
        where: { sessionId_studentId: { sessionId: session.id, studentId: dto.studentId } },
        create: {
  ...
  async bulkMark(actorId: string, dto: BulkAttendanceDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const activeStudents = await tx.enrollment.findMany({
        where: { groupId: session.groupId, status: EnrollmentStatus.ACTIVE, deletedAt: null },
      ...
      for (const it of dto.items) {
        ...
        await tx.attendance.upsert({
          where: { sessionId_studentId: { sessionId: session.id, studentId: it.studentId } },
          create: {
            sessionId: session.id,
  ...
  async monthlyGrid(q: AttendanceMonthlyDto) {
    ...
    const enrolls = await this.prisma.enrollment.findMany({
      where: { groupId: q.groupId, status: EnrollmentStatus.ACTIVE, deletedAt: null },
    ...
    const attendance = sessionIds.length
      ? await this.prisma.attendance.findMany({
          ...
          select: { sessionId: true, studentId: true, status: true, comment: true },
        })
      : [];
